...
# Importar o model Mensagem do Django
...
class rag():
  ...
  def carregar_llm(self):
    ...
    llm = OpenAI(
        model="gpt-4-mini",  # ou "gpt-4", "gpt-3.5-turbo"
  ...
  def salvar_mensagem(self, chat_id, role, conteudo, pergunta_original=None, pergunta_processada=None, intencao=None):
    """
    Salva uma mensagem no banco de dados usando o model Mensagem
    """
  ...
  def fazer_pergunta(self, chat_id, pergunta_usuario):
    """
    Processa a pergunta do usuário e retorna a resposta, salvando tudo no banco
    """
